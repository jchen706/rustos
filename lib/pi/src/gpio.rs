...
impl Gpio<Uninitialized> {
    ...
    pub fn into_alt(self, function: Function) -> Gpio<Alt> {
        //fsel register number
        //fsel pin corresponding to the register
        let alt_register = self.pin / 10;
        let function_number = self.pin % 10;
        let mask = !(0b111 << (function_number * 3));
        let register_bits = (function as u32) << (function_number * 3);

        let next_value: u32 = (self.registers.FSEL[alt_register as usize].read() & mask) | register_bits;
        self.registers.FSEL[alt_register as usize].write(next_value);
        self.transition()
    }
...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
       let set_register = self.pin / 32;
       let shift = self.pin % 32;
       self.registers.SET[set_register as usize].write(1 << shift);
    }
    ...
    pub fn clear(&mut self) {
        let set_register = self.pin / 32;
        let shift = self.pin % 32;
        self.registers.CLR[set_register as usize].write(1 << shift);
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        ...
        let val:u32 = self.registers.LEV[set_register as usize].read() & !(1 << shift);
