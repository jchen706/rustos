...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte_or_cancel(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let single_byte = self.read_byte(false)?;

        if single_byte == byte {
            Ok(single_byte)
        } else if (single_byte == CAN) && (single_byte != byte) {
            self.write_byte(CAN)?;
            return ioerr!(ConnectionAborted, "received CAN");
        } else {
            self.write_byte(CAN)?;
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        let single_byte = self.read_byte(byte!=CAN)?;
        if single_byte == byte {
            Ok(single_byte)
        } else if (single_byte == CAN) && (single_byte != byte) {
            return ioerr!(ConnectionAborted, "received CAN");
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let byte_from_sender:u8 = self.read_byte(true)?;
        //println!("{}",byte_from_sender);

        if byte_from_sender != SOH && byte_from_sender != EOT {
            //end transmissions cancel

            //println!("{}","notsohandeot");
            //self.write_byte(CAN);
            return ioerr!(InvalidData, "Expected SOH or EOT");

        } else if byte_from_sender == EOT {
            //end transmissions
            //println!("{}","eotread");
            self.write_byte(NAK)?;
        ...
        //println!("{}","aftereotandsot");

        //not EOT
        self.expect_byte_or_cancel(self.packet,"Expected Same Packet Number")?;
        self.expect_byte_or_cancel(255 - self.packet,"Expected A Ones Complement of Packet")?;
        ...
        for byte in buf {
            *byte = self.read_byte(false)?;
             ...
             //((*byte+checksum) as u16 % 256) as u8;
         ...
         let checksum1:u8 = (checksum % 256) as u8;
         //let checksum = get_checksum(buf);

        //mod checksum by 265
        let realchecksum = self.read_byte(false)?;

        if realchecksum != checksum1 {
            self.write_byte(NAK)?;
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if self.started==false {
            ...
            self.expect_byte(NAK, "NAK is expected at waiting")?;
            // let first = self.read_byte(false)?;
            // if first != NAK {
            //     self.write_byte(CAN)?;
            //     return ioerr!(InvalidData, "First byte isn't a NAK");
            //}
            self.packet = 1;
            self.started =  true;
            (self.progress)(Progress::Started);
        ...
        if buf.len() == 0 {
            ...
            self.write_byte(EOT)?;
            self.expect_byte(ACK, "Expecting a ACK")?;
            self.started = false;
            return Ok(0);
        }
        ...
        //println!("{}","check complement");


        //println!("{}", "writebeforechecksum");
        let mut checksum: u64 = 0;
        for byte in buf {
            self.write_byte(*byte)?; // Write packet byte
            // println!("{}", byte);
             checksum += *byte as u64
        ...
        //let checksum = get_checksum(buf);
        //println!("{}", "writeafterchecksum");


        //println!("{}", checksum);

        self.write_byte(checksum1)?;
        //println!("{}", checksum);
        let byte_read = self.read_byte(true)?;
        //println!("{}", "byte_readiwrite");
        if byte_read == NAK {

            return ioerr!(Interrupted, "Checksum failed. Retry");
        } else if byte_read == ACK {
            ...
            return ioerr!(InvalidData, "Expected ACK");
        }
